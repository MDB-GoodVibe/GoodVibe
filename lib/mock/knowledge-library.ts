...
const tipsArticles: ArticleSeedInput[] = [
  ...
  {
    slug: "getdesign-md-practical-guide",
    title: "getdesign.md로 AI UI 디자인 일관성 빠르게 잡기",
    summary:
      "getdesign.md의 DESIGN.md를 프로젝트에 연결하면 AI 코딩 에이전트가 색상, 타이포, 컴포넌트 스타일을 더 일관되게 생성할 수 있습니다.",
    track: "tips",
    topic: "examples-and-showcase",
    featured: true,
    platformTags: ["Web"],
    toolTags: ["getdesign.md", "DESIGN.md", "UI", "Vibe Coding"],
    resourceUrl: "https://getdesign.md/",
    sections: [
      {
        heading: "getdesign.md가 정확히 뭐냐",
        blocks: [
          "getdesign.md는 공개 웹사이트에서 관찰 가능한 디자인 패턴을 정리한 DESIGN.md 컬렉션을 탐색할 수 있는 디렉토리입니다. 핵심 목적은 '복붙 클론'이 아니라, AI 코딩 에이전트가 참고할 디자인 언어를 빠르게 주입하는 것입니다.",
          bullets([
            "디자인 토큰: 색상, 타이포, 간격, 반경, 그림자",
            "컴포넌트 패턴: 버튼, 카드, 폼, 내비게이션",
            "레이아웃 패턴: 섹션 밀도, 정보 계층, 반응형 흐름",
            "브랜드 톤: 무드, 카피 길이, 시각적 강조 방식",
          ]),
          "중요 포인트: getdesign.md의 DESIGN.md는 공식 브랜드 시스템 문서가 아니라, 공개 UI를 바탕으로 정리된 실무 참고 자료입니다.",
        ],
      },
      {
        heading: "왜 바이브 코딩에서 효과가 큰가",
        blocks: [
          "바이브 코딩이 흔히 망가지는 지점은 '한 화면은 괜찮은데 전체가 제각각'인 상태입니다. DESIGN.md를 먼저 넣으면 에이전트가 공통 기준을 가진 상태로 코드를 생성하기 때문에, 화면이 늘어나도 스타일 일관성이 유지됩니다.",
          bullets([
            "속도: 매번 디자인 설명을 반복하지 않아도 됨",
            "일관성: 컬러/타이포/간격 규칙이 통일됨",
            "수정 용이성: 한 기준 문서만 고치면 전반 수정 가능",
            "협업성: 팀원이 바뀌어도 동일한 디자인 문맥 공유 가능",
          ]),
        ],
      },
      {
        heading: "실전 적용 순서 (권장 워크플로우)",
        blocks: [
          steps([
            "getdesign.md에서 프로젝트 성격에 맞는 레퍼런스를 1~2개 고릅니다.",
            "DESIGN.md를 프로젝트 루트에 둡니다. (예: /DESIGN.md)",
            "Vibe Helper Stage 1 또는 superpowers 단계에서 'DESIGN.md를 반드시 읽고 준수'를 명시합니다.",
            "첫 구현은 홈/리스트/상세처럼 대표 화면 2~3개만 생성합니다.",
            "생성 결과를 보고 토큰(색/간격/타입)만 먼저 조정합니다.",
            "토큰 기준이 안정되면 나머지 페이지를 확장합니다.",
          ]),
          "핵심은 '처음부터 완벽한 화면'이 아니라 '확장 가능한 디자인 기준 확정'입니다.",
        ],
      },
      {
        heading: "에이전트 프롬프트 템플릿 (바로 사용)",
        blocks: [
          "아래 템플릿을 Stage 1 또는 화면 구현 시작 프롬프트에 붙이면 효과가 좋습니다.",
          "```md\n너는 프론트엔드 구현 전에 반드시 `/DESIGN.md`를 먼저 읽어라.\n\n규칙:\n1) 색상/타이포/간격/반경/그림자 토큰을 우선 추출해 요약한다.\n2) 컴포넌트 스타일(버튼/카드/입력폼/탭/배지)을 DESIGN.md 기준으로 통일한다.\n3) 새로운 UI를 만들 때도 기존 토큰과 패턴만 사용한다.\n4) 불확실한 디자인 결정은 임의로 만들지 말고 질문으로 확인한다.\n\n산출물:\n- design-summary.md (토큰/컴포넌트 규칙 요약)\n- 구현 코드\n- 디자인 규칙 위반 가능성 체크리스트\n```",
          "이 템플릿의 의도는 '먼저 기준 문서 요약 -> 그다음 코드 생성' 순서를 강제하는 것입니다.",
        ],
      },
      {
        heading: "자주 하는 실수와 해결법",
        blocks: [
          bullets([
            "실수 1: 레퍼런스를 4~5개 섞음 -> 해결: 처음엔 1개, 많아도 2개만",
            "실수 2: 토큰보다 화면부터 손댐 -> 해결: 토큰 정리 후 컴포넌트 확장",
            "실수 3: 에이전트에게 '느낌만 비슷하게' 요청 -> 해결: 금지/허용 규칙 명시",
            "실수 4: 반응형 기준 누락 -> 해결: 모바일/태블릿/데스크톱 기준 먼저 합의",
            "실수 5: 카피 톤 미정 -> 해결: 버튼/헤드라인 문장 길이 규칙까지 문서화",
          ]),
          "특히 실무에서 많이 터지는 문제는 '디자인은 맞는데 UX 밀도가 안 맞는' 경우입니다. 이때는 컴포넌트보다 섹션 간 간격과 정보 우선순위(시선 흐름)부터 맞추는 것이 빠릅니다.",
        ],
      },
      {
        heading: "Vibe Helper와 연결하는 방법",
        blocks: [
          steps([
            "Superpowers 단계에서 프로젝트 성격과 레퍼런스 무드를 질문/합의합니다.",
            "Stage 1에서 `project-brief.md`와 함께 `design-summary.md` 생성을 요구합니다.",
            "Stage 2 UI 설계는 `project-brief.md + design-summary.md`를 참조하게 합니다.",
            "Stage 3~4에서도 새 화면/랜딩/대시보드가 생기면 같은 토큰을 재사용하게 강제합니다.",
          ]),
          "한 줄 정리: getdesign.md는 '예쁜 샘플 모음'이 아니라, 에이전트에게 일관된 디자인 추론 프레임을 주는 기준점입니다.",
        ],
      },
      {
        heading: "운영 관점 체크리스트",
        blocks: [
          bullets([
            "디자인 기준 파일 위치가 고정되어 있는가 (/DESIGN.md 등)",
            "에이전트가 먼저 문서를 읽고 요약하도록 프롬프트가 구성되어 있는가",
            "팀이 공통으로 쓰는 토큰 명명 규칙이 있는가",
            "새 화면 추가 시 기존 패턴 재사용을 검증하는 체크가 있는가",
            "브랜드 리뉴얼 시 DESIGN.md 업데이트 절차가 있는가",
          ]),
          "이 체크리스트를 루틴으로 만들면 화면 개수가 늘어나도 디자인 품질이 무너지지 않습니다.",
        ],
      },
    ],
  },

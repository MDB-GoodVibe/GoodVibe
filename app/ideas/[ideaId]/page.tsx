    ...
    <div className="grid gap-8 xl:grid-cols-[minmax(0,1fr)_320px]">
      <article className="min-w-0 space-y-8">
        <header className="rounded-[2rem] border border-[rgba(121,118,127,0.08)] bg-white px-7 py-8 shadow-[0_18px_38px_rgba(37,31,74,0.05)]">
          ...
          <div className="mt-5 space-y-4">
            ...
            <p className="max-w-4xl text-[15px] leading-8 whitespace-pre-wrap text-foreground/92">
              {idea.content}
            </p>
          </div>
        </header>
      ...
      <aside className="space-y-4">
        <div className="rounded-[1.9rem] bg-secondary px-6 py-6 text-white shadow-[0_22px_46px_rgba(176,72,72,0.18)]">
          <p className="text-sm font-semibold uppercase tracking-[0.18em] text-white/72">참여하기</p>
          <p className="mt-3 text-sm leading-7 text-white/76">
            공감이 가는 아이디어라면 투표하고, 바로 헬퍼로 가져가 구조 초안까지 만들어 보세요.
          </p>

          <div className="mt-5 grid gap-2.5">
            <form action={toggleIdeaVoteAction}>
              <input type="hidden" name="ideaId" value={idea.id} />
              <Button type="submit" className="w-full bg-white text-secondary hover:bg-white/92">
                <ThumbsUp className="size-4" />
                {idea.viewerHasVoted ? "투표 취소" : "투표하기"}
              </Button>
            </form>

            <Button asChild variant="outline" className="w-full border-white/18 bg-white/10 text-white hover:bg-white/16">
